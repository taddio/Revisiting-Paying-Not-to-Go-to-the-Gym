...
Club_site_expanded = Club_site_expanded[, -1]

Customer_agreement = read_csv("Customer_Agreement.csv") #Read Customer Agreement Data
Customer_agreement = Customer_agreement[, -2]

length(unique(Customer_agreement$participant_id[which(Customer_agreement$participant_id %in% unique(participant_checkin$participant_id))]))

Gym_customer = read_csv("Gym_Customer.csv") #Read Gym Customer data

Customer_agreement_subset_3 = read_csv("Customer_agreement_subset_3.csv") #Read Gym Customer data
Customer_agreement_subset_3 = Customer_agreement_subset_3[, -c(1,2)]

upgrade_customer_agreements = read_csv("Upgrade_customer_agreements.csv")
...
#Finding number of attendances at each club
club_checkins = participant_checkin_2$club_num
club_nums = Club_site_expanded$club_src_num
club_attendences = c()
for(i in 1:length(club_nums)){ #For every club, find number of checkins in participant checkin data using which()
  indices = which(club_checkins == club_nums[i])
...
Club_site_expanded$total_subset_attendance = club_attendences
...
club_nums = Club_site_2$club_src_num

#finding minimum dates
min_dates = c()

for(i in 1:length(club_nums)){
  subset = participant_checkin[which(checkin_club_numbers == club_nums[i]),]
...
Club_site_2$max_checkin_date = max_dates
Club_site_2$min_checkin_date = min_dates

#Add boolean for opening during time period

start_date_boolean = c()
for(i in 1:length(club_nums)){
  if(min_dates[i] < "2010-01-01 00:00:00-08:00"){
    start_date_boolean = c(start_date_boolean, 0)
  }
  else{
    start_date_boolean = c(start_date_boolean, 1)
  }
}

#Add boolean for closing during time period
end_date_boolean = c()
for(i in 1:length(club_nums)){
  if(max_dates[i] > "2017-01-01 00:00:00-08:00"){
    end_date_boolean = c(end_date_boolean, 0)
  }
  else{
    end_date_boolean = c(end_date_boolean, 1)
  }
}


Club_site_2$opened_after_2010 = start_date_boolean
Club_site_2$closed_before_2017 = end_date_boolean
...
participant_checkin_2 = participant_checkin[which(checkin_times < "2017-01-01 00:00:00-08:00"), ]

#7,787,522 attendances between 2010-2017

#Counting attendances of each person
attendance_table = table(participant_checkin_2$participant_id)
attendance_table = as.data.frame(attendance_table)
...
#Finding duration of memberships - core type 17
...
for(i in 1:length(Customer_agreement_types)){
  if(Customer_agreement_types[i] == 17 & (Customer_agreement_statuses[i] == 3 | 
                                                 Customer_agreement_statuses[i] == 5)){
    indices = c(indices, i)
  }
}

core_customer_agreements = Customer_agreement[indices, ]
core_customer_agreements = core_customer_agreements[, -c(8:11)]

core_customer_agreements$start_date = as.Date("2010-01-01", "%Y-%m-%d")
core_customer_agreements$start_date = as.Date(core_customer_agreements$start_date, "%Y-%m-%d")
core_customer_agreements$end_date = as.Date("2017-01-01", "%Y-%m-%d")
...
new_dates = core_customer_agreements$start_date
...
new_dates_2 = core_customer_agreements$end_date
...
core_customer_agreements$date_diff_month = core_customer_agreements$date_diff/30.44
...
core_customer_agreements_2 = core_customer_agreements[-which(core_customer_agreements$date_diff == 0), ]
...
which(participant_checkin_ids == "2baac446-1f7a-4a6f-8691-c73576278edc")
...
sum(core_customer_agreements_2$upgrade_boolean)
...
upgrade_duration_all

which(upgrade_duration_all > 2557)
participant_ids[192]
...
single_club #4769 only attended a single club

upgrade_customer_agreements_2 = upgrade_customer_agreements[which(upgrade_customer_agreements$date_diff > 0), ]

upgrade_customer_agreements_2 = upgrade_customer_agreements_2[-which(duplicated(upgrade_customer_agreements_2) == TRUE), ]
...
core_customer_agreements_2$upgrate_duration_months = core_customer_agreements_2$upgrade_duration/30.45
...
sqrt(var(average_cost_attendances))
...
for(i in 20217:length(participant_ids)){
