...
dataset = fread("participant_checkin_2.csv")
dataset = dataset[, -1]
...
dataset$club_type_id = as.integer(Club_site$club_type_id)  

#-----------------------------------------

#Using new ID numbers for dataset
club_nums = dataset$club_num
club_types = dataset$club_num
...
dataset$simple_participant_id = 0
simple_participant_id = dataset$simple_participant_id
participant_id_list = dataset$participant_id


participant_id = dataset$participant_id[1] #starting from first ID
integer = 1
...
dataset$simple_participant_id = simple_participant_id
...
setkey(dataset, simple_participant_id)


participant = 1
participant_membership = c()
for(i in 1:72339){ #72339 different particpants
  participant = dataset[simple_participant_id == i] #Subset for each particpant
...
dataset$membership_type = participant_membership
...
checkin_times = dataset$checkin_datetime_localtime
...
date_diff = c()
final_date = "2019-02-17"
for(i in 1:(length(start_dates))){
  if(is.na(end_dates[i])){
    date_diff = c(date_diff, as.Date(as.character(final_date), format="%Y/%m/%d")-
                    as.Date(as.character(start_dates[i]), format="%Y/%m/%d"))
  }
  else{
    date_diff = c(date_diff, as.Date(as.character(end_dates[i]), format="%Y/%m/%d")-
    as.Date(as.character(start_dates[i]), format="%Y/%m/%d"))
  }
}
...
#--------------------------------------------------
#Save Files
write.csv(dataset, "participant_checkin_2.csv")
