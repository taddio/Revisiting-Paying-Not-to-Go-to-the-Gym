...
participant_checkin_subset_2 = fread("participant_checkin_subset_2.csv")
participant_checkin_subset_2 = participant_checkin_subset_2[, -1]
...
#Finding Length of Contracts
start_dates = subset_customer_agreement$cleaned_agreement_sign_dt
end_dates = subset_customer_agreement$cleaned_termination_date

date_diff = c()
final_date = "2017-01-01" #Final attendance entry in dataset
for(i in 1:(length(start_dates))){
  if(is.na(end_dates[i])){
    date_diff = c(date_diff, difftime(final_date, start_dates[i]))
  }
...
art_cutoff = subset_customer_agreement$cleaned_termination_date
